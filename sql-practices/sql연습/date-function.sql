...
select sysdate(), sleep(2), sysdate(); -- sysdatae 시스템이 실행한 시간 now랑 다름

-- date_format
-- 2022년 05월 13일 11:39:40 
...
select date_format('2022-10-10', '%Y년 %m월 %d일'), 
	date_format(cast('2022-10-10' as date), '%Y년 %m월 %d일');
select cast(cast(1-2 as unsigned) as signed);

-- #### mysql type #####
-- varchar, char, text, CLOB(Character Large Object)
-- signed(unsigned), int(integer), medium int, bin int, int(11)
-- float, double
-- time, date, datetime
